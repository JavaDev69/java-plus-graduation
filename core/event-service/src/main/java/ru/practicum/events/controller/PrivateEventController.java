...
public class PrivateEventController {
    ...
    public List<EventFullDto> getUserModerationHistory(
        ...
        log.info("Получен запрос на получения истории модерации пользователя с ID: {}, from: {}, size: {}", userId, from, size);
        List<EventFullDto> events = eventsService.getUserModerationHistory(userId, from, size);
        log.info("Для пользователя с ID {} найдено {} записей", userId, events.size());
        return events;
    }
